...
public class ScanPayController {
    ...
    public void unifiedPay(WechatScanPayRequest wechatScanPayRequest) throws Exception {
        /* 初始化证书，发送报文 */
        HttpsRequest.init(false, wechatScanPayRequest.getCertPath(), wechatScanPayRequest.getCertPassword());

        String unifiedResponse = HttpsRequest.sendPost(wechatConfig.getPayApi(), wechatScanPayRequest);
